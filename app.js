...
// NOTA CRÍTICA: Supabase s'inicialitza dins DOMContentLoaded per evitar
// errors de timing que trenquen tot el codi (incloent el menú lateral)
...
async function loadAll(){
  if(!supabase){showToast('Configura les credencials de Supabase!','error');showLoadError();return;}
  try{
    const [s,t,c,p,e,k]=await Promise.all([
      supabase.from('students').select('*').order('name'),
      supabase.from('teachers').select('*').order('name'),
      supabase.from('classes').select('*').order('day_order').order('start_time'),
    ...
    if(p.error)throw p.error;if(e.error)throw e.error;if(k.error)throw k.error;
    students=s.data||[];teachers=t.data||[];classes=c.data||[];
    payments=p.data||[];expenses=e.data||[];tasks=k.data||[];
    dataLoaded=true;
    await updatePaymentStatuses();
    loadDashboard();
  }catch(err){console.error(err);showToast('Error: '+(err.message||err),'error');showLoadError();}
}

function showLoadError(){
  const g=document.getElementById('statsGrid');
  if(g)g.innerHTML=`<div class="stat-card" style="grid-column:1/-1;text-align:center;padding:2rem;color:var(--text-sec)">⚠️ Error carregant dades. Revisa Supabase.</div>`;
}
...
async function updatePaymentStatuses(){
  ...
  try{await supabase.from('payments').update({status:'vençut'}).in('id',ids);payments.forEach(p=>{if(ids.includes(p.id))p.status='vençut';});}
...
async function saveStudent(){
  ...
  try{if(editingId){const{error}=await supabase.from('students').update(data).eq('id',editingId);if(error)throw error;students=students.map(s=>s.id===editingId?{...s,...data}:s);showToast('Alumne actualitzat ✓');}else{const{data:res,error}=await supabase.from('students').insert(data).select().single();if(error)throw error;students.push(res);showToast('Alumne afegit ✓');}closeModal();renderStudents();}catch(e){showToast('Error: '+e.message,'error');}
}
async function deleteStudent(id){
  const s=students.find(x=>x.id===id);
  confirmAction(`Eliminar "${s?.name} ${s?.surname}"?`,async()=>{try{const{error}=await supabase.from('students').delete().eq('id',id);if(error)throw error;students=students.filter(x=>x.id!==id);payments=payments.filter(x=>x.student_id!==id);renderStudents();showToast('Alumne eliminat');}catch(e){showToast('Error: '+e.message,'error');}});
}
...
async function saveTeacher(){
  ...
  if(!data.name||!data.surname){showToast('Nom i cognoms obligatoris','error');return;}
  try{if(editingId){const{error}=await supabase.from('teachers').update(data).eq('id',editingId);if(error)throw error;teachers=teachers.map(t=>t.id===editingId?{...t,...data}:t);showToast('Professor actualitzat ✓');}else{const{data:res,error}=await supabase.from('teachers').insert(data).select().single();if(error)throw error;teachers.push(res);showToast('Professor afegit ✓');}closeModal();renderTeachers();}catch(e){showToast('Error: '+e.message,'error');}
}
async function deleteTeacher(id){
  const t=teachers.find(x=>x.id===id);
  confirmAction(`Eliminar "${t?.name} ${t?.surname}"?`,async()=>{try{const{error}=await supabase.from('teachers').delete().eq('id',id);if(error)throw error;teachers=teachers.filter(x=>x.id!==id);renderTeachers();showToast('Professor eliminat');}catch(e){showToast('Error: '+e.message,'error');}});
}
...
function renderSchedule(){
  ...
  const tF=tSel?.value||'',sF=sSel?.value||'';
  const todayIdx=TODAY.getDay()===0?7:TODAY.getDay();
...
async function saveClass(){
  ...
  try{if(editingId){const{error}=await supabase.from('classes').update(data).eq('id',editingId);if(error)throw error;classes=classes.map(c=>c.id===editingId?{...c,...data}:c);showToast('Classe actualitzada ✓');}else{const{data:res,error}=await supabase.from('classes').insert(data).select().single();if(error)throw error;classes.push(res);showToast('Classe afegida ✓');}closeModal();renderSchedule();}catch(e){showToast('Error: '+e.message,'error');}
}
async function deleteClass(id){
  confirmAction('Eliminar aquesta classe?',async()=>{try{const{error}=await supabase.from('classes').delete().eq('id',id);if(error)throw error;classes=classes.filter(c=>c.id!==id);closeModal();renderSchedule();showToast('Classe eliminada');}catch(e){showToast('Error: '+e.message,'error');}});
}
...
async function savePayment(){
  ...
  try{if(editingId){const{error}=await supabase.from('payments').update(data).eq('id',editingId);if(error)throw error;payments=payments.map(p=>p.id===editingId?{...p,...data}:p);showToast('Pagament actualitzat ✓');}else{const{data:res,error}=await supabase.from('payments').insert(data).select().single();if(error)throw error;payments.push(res);showToast('Pagament afegit ✓');}closeModal();renderPayments();}catch(e){showToast('Error: '+e.message,'error');}
}
async function markPaid(id){
  try{const td=todayStr();const{error}=await supabase.from('payments').update({status:'pagat',paid_date:td}).eq('id',id);if(error)throw error;payments=payments.map(p=>p.id===id?{...p,status:'pagat',paid_date:td}:p);renderPayments();showToast('Pagament marcat com a pagat ✓');}catch(e){showToast('Error: '+e.message,'error');}
}
async function deletePayment(id){
  confirmAction('Eliminar aquest pagament?',async()=>{try{const{error}=await supabase.from('payments').delete().eq('id',id);if(error)throw error;payments=payments.filter(p=>p.id!==id);renderPayments();showToast('Pagament eliminat');}catch(e){showToast('Error: '+e.message,'error');}});
}
...
async function saveExpense(){
  ...
  try{if(editingId){const{error}=await supabase.from('expenses').update(data).eq('id',editingId);if(error)throw error;expenses=expenses.map(e=>e.id===editingId?{...e,...data}:e);showToast('Gasto actualitzat ✓');}else{const{data:res,error}=await supabase.from('expenses').insert(data).select().single();if(error)throw error;expenses.push(res);showToast('Gasto afegit ✓');}closeModal();renderFinances();}catch(e){showToast('Error: '+e.message,'error');}
}
async function deleteExpense(id){
  confirmAction('Eliminar aquest gasto?',async()=>{try{const{error}=await supabase.from('expenses').delete().eq('id',id);if(error)throw error;expenses=expenses.filter(e=>e.id!==id);renderFinances();showToast('Gasto eliminat');}catch(e){showToast('Error: '+e.message,'error');}});
}
...
async function quickAddTask(){
  ...
  try{const{data:res,error}=await supabase.from('tasks').insert(data).select().single();if(error)throw error;tasks.unshift(res);if(el)el.value='';renderTasks();showToast('Tasca afegida ✓');}catch(e){showToast('Error: '+e.message,'error');}
}
...
async function saveTask(){
  ...
  try{if(editingId){const{error}=await supabase.from('tasks').update(data).eq('id',editingId);if(error)throw error;tasks=tasks.map(t=>t.id===editingId?{...t,...data}:t);showToast('Tasca actualitzada ✓');}else{const{data:res,error}=await supabase.from('tasks').insert(data).select().single();if(error)throw error;tasks.unshift(res);showToast('Tasca afegida ✓');}closeModal();renderTasks();}catch(e){showToast('Error: '+e.message,'error');}
}
async function toggleTask(id,cur){
  const ns=cur==='completada'?'pendent':'completada';
  try{const{error}=await supabase.from('tasks').update({status:ns}).eq('id',id);if(error)throw error;tasks=tasks.map(t=>t.id===id?{...t,status:ns}:t);renderTasks();if(ns==='completada')showToast('Tasca completada! ✓');}catch(e){showToast('Error: '+e.message,'error');}
}
async function deleteTask(id){
  confirmAction('Eliminar aquesta tasca?',async()=>{try{const{error}=await supabase.from('tasks').delete().eq('id',id);if(error)throw error;tasks=tasks.filter(t=>t.id!==id);renderTasks();showToast('Tasca eliminada');}catch(e){showToast('Error: '+e.message,'error');}});
}

// ── INIT ───────────────────────────────────────────────────
document.addEventListener('DOMContentLoaded', function() {

  // Supabase DINS DOMContentLoaded - evita errors de timing
  try {
